...
export class Line extends Pen {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (
      (!this.isAnimate || this.animateType !== "comet") &&
      drawLineFns[this.name]
    ) {
      if (this.lineJoin) {
        ctx.lineJoin = this.lineJoin;
      }
    ...
    if (this.fromArrow && drawArrowFns[this.fromArrow]) {
      ...
      ctx.setLineDash([]);
      ctx.fillStyle =
        this.fromArrowColor || this.strokeStyle || ctx.strokeStyle;
  ...
  animate(now: number) {
    ...
    switch (this.animateType) {
      ...
      case "comet":
        this.lineDash = null;
        this.animateDot = this.getPointByPos(
          this.animatePos + this.animateFromSize
        );
        break;
      default:
